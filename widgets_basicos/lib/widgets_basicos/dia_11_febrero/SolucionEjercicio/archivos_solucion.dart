...
/// ════════════════════════════════════════════════════════════════════════════
/// ALMACENAMIENTO EN ARCHIVOS - SOLUCIÓN EDUCATIVA
/// ════════════════════════════════════════════════════════════════════════════
...
class Tarea {
  ...
  /// ✓ ERROR 1 CORREGIDO: Cambiar id de String a int
  factory Tarea.fromJson(Map<String, dynamic> json) => Tarea(
    id: json['id'] as int, // ✓ CORRECCIÓN: Cambiar de String a int
...
class AlmacenamientoTareas {
  ...
  /// ✓ ERROR 2 CORREGIDO: Agregar await a writeAsString()
  static Future<void> guardarTareaJSON(Tarea tarea) async {
    try {
      ...
      await archivo.writeAsString(json); // ✓ CORRECCIÓN: Se agregó await
  ...
  /// ✓ ERROR 3 CORREGIDO: Invertir lógica del null check
  static Future<Tarea?> leerTareaJSON() async {
    try {
      final dir = await _directorio;
      final archivo = File('${dir.path}/tarea_actual.json');

      if (!await archivo.exists()) { // ✓ CORRECCIÓN: Cambiar await archivo.exists() a !await archivo.exists()
        return null;
      }
  ...
  /// ✓ ERROR 4 CORREGIDO: Cambiar cast de String a List
  static Future<List<Tarea>> leerTareasJSON() async {
    try {
      ...
      final contenido = await archivo.readAsString();
      final jsonList = jsonDecode(contenido) as List; // ✓ CORRECCIÓN: Cambiar 'as String' a 'as List'
  ...
  /// ✓ ERROR 5 CORREGIDO: El loop debe comenzar en 1, no en 0
  static Future<List<Tarea>> leerTareasCSV() async {
    try {
      ...
      for (int i = 1; i < lineas.length; i++) { // ✓ CORRECCIÓN: Cambiar de 'i = 0' a 'i = 1'
  ...
  /// ✓ ERROR 6 CORREGIDO: Agregar parámetro 'line' faltante
  static List<String> _parseCSVLine(String line) { // ✓ CORRECCIÓN: Agregar parámetro 'String line'
    final campos = <String>[];
    final buffer = StringBuffer();
