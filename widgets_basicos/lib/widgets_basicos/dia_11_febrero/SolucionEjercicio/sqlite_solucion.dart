...
class Calificacion {
  ...
  /// ✓ ERROR 1 CORREGIDO: Cambiar conversión de nota a double
  factory Calificacion.fromMap(Map<String, dynamic> map) {
    return Calificacion(
      id: map['id'],
      estudianteId: map['estudianteId'] ?? 0,
      asignatura: map['asignatura'] ?? '',
      nota: (map['nota'] ?? 0.0).toDouble(), // ✓ CORRECCIÓN: Cambiar toInt() a toDouble()
...
class AlmacenamientoSQLite {
  ...
  /// ✓ ERROR 2 CORREGIDO: Agregar await
  static Future<int> agregarCalificacion(Calificacion calificacion) async {
    final db = await obtenerBD();
    int id = await db.insert('calificaciones', calificacion.toMap()); // ✓ CORRECCIÓN: Se agregó await
    print('✓ Calificación agregada');
    return id;
  }

  /// Obtener calificaciones de un estudiante
  /// ✓ ERROR 3 CORREGIDO: Cambiar tipo de retorno a List<Calificacion>
  static Future<List<Calificacion>> obtenerCalificacionesEstudiante(
    int estudianteId,
  ) async {
    final db = await obtenerBD();
    final result = await db.query(
    ...
    return result.map((map) => Calificacion.fromMap(map)).toList(); // ✓ CORRECCIÓN: Cambiar tipo de retorno
  }
  ...
  /// ✓ ERROR 4 CORREGIDO: Invertir lógica del check
  static Future<double> obtenerPromedio(int estudianteId) async {
    ...
    if (result.isEmpty) return 0.0; // ✓ CORRECCIÓN: Cambiar isNotEmpty a isEmpty
    return (result.first['promedio'] as num?)?.toDouble() ?? 0.0;
  }

  /// Obtener estudiantes con promedio mayor a X
  /// ✓ ERROR 5 CORREGIDO: Cambiar tipo de parámetro a double
  static Future<List<Map>> obtenerEstudiantesConPromedioAlto(
    double minimo, // ✓ CORRECCIÓN: Cambiar String a double
  ) async {
    final db = await obtenerBD();
...
class _PantallaSQLiteState extends State<PantallaSQLite> {
      ...
      body: estudiantes.isEmpty
          ...
          : ListView.builder(
              ...
              itemBuilder: (context, index) {
                return _buildEstudianteCard(estudiantes[index], context);
              },
            ),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          ElevatedButton.icon(
            onPressed: (){_mostrarDialogoAgregarEstudiante(context);},
  ...
  Widget _buildEstudianteCard(Estudiante estudiante, BuildContext context) {
    return Card(
          ...
          children: [
            Row(
              ...
              children: [
                ...
                /// ✓ ERROR 6 CORREGIDO: Remover onPressed duplicado
                ElevatedButton.icon(
                  onPressed: () async {
            ...
            Row(
              ...
              children: [
                TextButton.icon(
                  onPressed: () {
                    _mostrarDialogoAgregarCalificacion(estudiante.id!, context);
                  },
  ...
  void _mostrarDialogoAgregarEstudiante(BuildContext context) {
  ...
  void _mostrarDialogoAgregarCalificacion(int estudianteId, BuildContext context) {
