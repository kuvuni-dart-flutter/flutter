...
/// ════════════════════════════════════════════════════════════════════════════
/// SHARED PREFERENCES - SOLUCIÓN EDUCATIVA
/// ════════════════════════════════════════════════════════════════════════════
///
/// ✓ TODAS LAS CORRECCIONES APLICADAS
/// Este archivo contiene las SOLUCIONES a todos los errores intencionales
///
/// ════════════════════════════════════════════════════════════════════════════
...
class AlmacenamientoPreferencias {
  ...
  /// Leer token
  /// ✓ ERROR 2 CORREGIDO: Ahora usa getString() en lugar de getInt()
  static Future<String> leerToken() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getString(_claveToken) ?? ''; // ✓ CORRECCIÓN: getString() en lugar de getInt()
  }
  ...
  /// ✓ ERROR 3 CORREGIDO: Se agregó await a guardarNotificaciones()
  static Future<void> guardarPreferencias(Preferencias prefs) async {
    ...
    await guardarNotificaciones(prefs.notificacionesHabilitadas); // ✓ CORRECCIÓN: Se agregó await
            ...
            children: [
              ListTile(
                ...
                trailing: Switch(
                  ...
                  /// ✓ ERROR 6 CORREGIDO: Ahora el callback es async
                  onChanged: (valor) async { // ✓ CORRECCIÓN: Se agregó async
                    final nuevoTema = valor ? 'dark' : 'light';
                    await AlmacenamientoPreferencias.guardarTema(nuevoTema); // ✓ CORRECCIÓN: Se agregó await
